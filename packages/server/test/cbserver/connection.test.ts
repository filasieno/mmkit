...
import {
  ...
  waitForConnectionBootstrap,
  waitForConnectionClose,
  waitForNotificationAnswer,
} from "../../src/cbserver/actors/connection/CBServerConnectionContext";
import {
  ConnectionIdle,
  ConnectionUninitialized,
} from "../../src/cbserver/actors/connection/CBServerConnectionActor";
...
import { makeMockCommandChannelPort, makeMockNotificationChannelPort } from "./mockTcpSocket";
...
describe("CBServerConnection [mock TCP socket]", function () {
  ...
  function makeConnectionContext(connectionId: string, onClose: () => void): CBConnectionContext {
    return new CBConnectionContext(
      connectionId,
      onClose,
      { host: "127.0.0.1", port: 4001 },
      {
        command: makeMockCommandChannelPort("mock-cmd-client"),
        notification: makeMockNotificationChannelPort("mock-notify-client"),
      },
    );
  }

  it("initialize enrolls both channels and reaches ConnectionIdle", async () => {
    const closed: string[] = [];
    const ctx = makeConnectionContext("conn-1", () => closed.push("closed"));
    const actor = ihsm.makeTestActor(CBConnectionTop, ctx, makeMockConnectionOrchestratorPort(), { initialize: false });
    actor.hsm.restore(ConnectionUninitialized, ctx);
  ...
  it("pwd sends GET_MODULE_PATH via command channel", async () => {
    ...
    const answerP = waitForCommandAnswer(ctx);
    actor.notify.dispatchPwd();
    const answer = await answerP;
    await actor.hsm.sync();

    expect(answer.ok).to.equal(true);
    const commandPort = ctx.channelPorts!.command! as ReturnType<typeof makeMockCommandChannelPort>;
  ...
  it("close sends CANCEL_ME on both channels and reaches ConnectionClosed", async () => {
    ...
    const cmdPort = ctx.channelPorts!.command! as ReturnType<typeof makeMockCommandChannelPort>;
    const notifPort = ctx.channelPorts!.notification! as ReturnType<typeof makeMockNotificationChannelPort>;
    cmdPort.destroy.default(() => {
      destroyOrder.push("command");
    });
    notifPort.destroy.default(() => {
      destroyOrder.push("notification");
    });
    const actor = ihsm.makeTestActor(CBConnectionTop, ctx, makeMockConnectionOrchestratorPort(), { initialize: false });
    actor.hsm.restore(ConnectionUninitialized, ctx);
    const bootstrap = waitForConnectionBootstrap(ctx);
    ...
    const closeP = waitForConnectionClose(ctx);
    actor.notify.close();
    await closeP;
    await actor.hsm.sync();
  ...
  it("reportClients uses REPORT_CLIENTS ipc method on command channel", async () => {
    ...
    const answerP = waitForCommandAnswer(ctx);
    actor.notify.dispatchReportClients();
    const answer = await answerP;
    await actor.hsm.sync();
  ...
  it("dispatches every command IPC via the command channel only", async () => {
    ...
    const stopAnswerP = waitForCommandAnswer(ctx);
    actor.notify.dispatchStopServer("");
  ...
  it("getNotificationMessage reads from the notification channel only", async () => {
    ...
    const answerP = waitForNotificationAnswer(ctx);
    actor.notify.dispatchGetNotificationMessage(500);
