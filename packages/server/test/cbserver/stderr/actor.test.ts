...
describe("CBServer stderr reader subactor", function () {
  ...
  it("spawn auto-starts in StderrIdle", async () => {
    const ctx = new StderrReaderContext();
    const actor = ihsm.makeTestActor(StderrLogReaderTop, ctx);
  ...
  it("onData emits complete lines via supervisor onStderrLine", async () => {
    ...
    const ctx = new StderrReaderContext(server);
    const actor = ihsm.makeTestActor(StderrLogReaderTop, ctx);
    await actor.hsm.sync();
