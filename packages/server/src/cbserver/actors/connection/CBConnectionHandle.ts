...
import { collectErrorReports, normalizeAskNilResult, tellTransactions } from "./utils";
...
export class CBConnectionHandle implements ICBConnection {
  ...
  async close(): Promise<void> {
    const done: Promise<void> = waitForConnectionClose(this.ctx);
  ...
  private async tellOne(frames: string): Promise<CBAnswer> {
    const answer: Promise<CBAnswer> = waitForCommandAnswer(this.ctx);
  ...
  async getNotificationMessage(timeoutMs = 0): Promise<CBAnswer> {
    const answer: Promise<CBAnswer> = waitForNotificationAnswer(this.ctx);
