/**
 * Connection client helpers — behaviors mirrored from Java `LocalCBclient` / `CBConnection`.
 */
import { decodeCbString } from "@mmkit/base";
import { IpcAnswer } from "../../shared/CBServerDefs";
import type { CBAnswer } from "../../shared/CBServerDefs";

const COMPLETION_RANK: Record<string, number> = {
  ok: 0,
  error: 1,
  notification: 2,
  not_handled: 3,
  timeout: 4,
  broken: 5,
};

/** CBanswer.mergeCBanswer — keep max severity completion, latest result/sender. */
export function mergeCbAnswers(current: CBAnswer, previous: CBAnswer | undefined): CBAnswer {
  if (previous === undefined) {
    return current;
  }
  const currentRank: number = COMPLETION_RANK[current.completion] ?? 0;
  const previousRank: number = COMPLETION_RANK[previous.completion] ?? 0;
  const completion: string = currentRank >= previousRank ? current.completion : previous.completion;
  return new IpcAnswer(current.term, completion === "ok", completion, current.result);
}

/**
 * LocalCBclient.tellTransactions — split `{---}` and normalize line endings (CRLF → LF).
 */
export async function tellTransactions( tellOne: (frames: string) => Promise<CBAnswer>, transactions: string ): Promise<CBAnswer> {
  const normalized: string = transactions.replace(/\r\n/g, "\n");
  if (!normalized.includes("{---}")) {
    return tellOne(normalized);
  }
  let merged: CBAnswer | undefined;
  const chunks: string[] = normalized.split("{---}");
  for (const chunk of chunks) {
    const trimmed: string = chunk.trim();
    if (trimmed === "") {
      continue;
    }
    const answer: CBAnswer = await tellOne(trimmed);
    merged = mergeCbAnswers(answer, merged);
  }
  return merged ?? new IpcAnswer("", true, "ok", "yes");
}

/**
 * LocalCBclient.getErrorMessages — drain ERROR_REPORT queue via NEXT_MESSAGE.
 */
export async function collectErrorReports(nextMessage: (type: string) => Promise<CBAnswer>,): Promise<CBAnswer> {
  const parts: string[] = [];
  let answer: CBAnswer = await nextMessage("ERROR_REPORT");
  while (answer.result !== "empty_queue") {
    let decoded: string | undefined;
    const result: string | undefined = answer.result;
    if (result !== undefined && result !== "empty_queue") {
      if (!result.startsWith("ipcmessage")) {
        decoded = result;
      } else {
        const bracketStart: number = result.indexOf("[");
        const bracketEnd: number = result.lastIndexOf("]");
        if (bracketStart >= 0 && bracketEnd > bracketStart) {
          const inner: string = result.slice(bracketStart + 1, bracketEnd).trim();
          if (inner === "") {
            decoded = "";
          } else {
            decoded = decodeCbString(inner) ?? inner;
          }
        }
      }
    }
    if (decoded !== undefined) {
      parts.push(decoded);
      answer = await nextMessage("ERROR_REPORT");
      continue;
    }
    break;
  }
  const text: string = parts.join("");
  return new IpcAnswer(text, true, "ok", text);
}

/** CBConnection.ask nil workaround for FRAME / LABEL answer representations. */
export function normalizeAskNilResult(answer: CBAnswer, answerRep?: string): CBAnswer {
  if (answer.result !== "nil") {
    return answer;
  }
  if (answerRep === "FRAME" || answerRep === "LABEL") {
    return new IpcAnswer(answer.term, answer.ok, answer.completion, "");
  }
  return answer;
}
